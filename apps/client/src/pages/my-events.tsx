...
import { WeekView } from "@/components/calendar/weak-view";
...
export function MyEvents() {
  ...
  if (isEventsLoading || isUserLoading) return <Loading />;
  ...
  if (events.length === 0) {
    return <NoEvents message="You are not part of any events yet" />;
  }
  ...
  return (
    <div className="w-full py-6 px-2 sm:px-0">
      <div className="mb-6 flex flex-col sm:flex-row text-center sm:text-start sm:justify-between gap-4">
        <PageHeader
          title="My Events"
          subtitle="View and manage your event calendar"
        />
        <Button
